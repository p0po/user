        ...
        for(int i = 0;i<joinPoint.getArgs().length;i++){
            ...
            if(!"".equals(regStr)){
                if(!(arg instanceof String)){
                    throw new ParamException("@Reg can only with String args in method "+controller.getName()+"."+methodName);
                }
                ...
                if(!matched){
                    throw new ParamException("args "+joinPoint.getArgs()[i].toString()+" of method "+controller.getName()+"."+methodName+" is not matched with Red "+regStr);
                }
            }
